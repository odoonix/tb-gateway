...
def equal_packet(packet_send, packet_save):
    if (
        packet_save["telemetry"] == packet_send["telemetry"]
        and packet_save["attributes"] == packet_send["attributes"]
    ):
        return True
    else:
        return False
    ...
    def __init__(self, gateway, config, connector_type):
        ...
        for key, config in self.printers.items():
            self.gateway.add_device(
                self.__prefix + key, {"connector": self}, device_type=self.__deviceType
            )
    ...
    def server_side_rpc_handler(self, content):
        ...
        try:
            ...
            self.gateway.send_rpc_reply(
                ...
                content={
                    "success_sent": "False",
                    "message": "Cups unsupported exception happend %s" % ex,
                },
            )
        finally:
            sem.release()
        ...
        try:
            if method_name == "print_file":
                ...
                try:
                    ...
                    self.print_file(
                        content["device"].replace(self.__prefix, ""), temp.name
                    )
            ...
            self.__logger.error(
                "Cups unsupported exception happend : %s, %s",
                ex,
