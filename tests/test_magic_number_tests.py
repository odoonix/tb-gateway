...
class TestStringMethods(unittest.TestCase):
    def test_convert_to_device_id(self):
        ...
        for user_id, device_id, magic_number in data:
            self.assertEqual(
                zktec_connector.convert_to_device_id(magic_number, user_id), device_id
            )
            self.assertEqual(
                zktec_connector.convert_to_company_id(magic_number, device_id), user_id
            )
    ...
    def test_run_single(self):
        ...
        connction._zkteco_get_attribute = MagicMock(
            return_value={
                "ZKTec Error": False,
                "Records": 100,
                "Max Records": 1000,
    ...
    def test_run_single_with_telemetry(self):
        ...
        connction._zkteco_get_attendance = MagicMock(
            return_value=[
                attendance.Attendance(
                    user_id=1,
                    timestamp=datetime(2022, 1, 1, 8, 0, 1),
                    status=1,
                    punch=0,
                    uid=1,
                ),
                attendance.Attendance(
                    user_id=1,
                    timestamp=datetime(2022, 1, 1, 9, 0, 1),
                    status=1,
                    punch=1,
                    uid=1,
                ),
                attendance.Attendance(
                    user_id=1,
                    timestamp=datetime(2022, 1, 1, 10, 0, 1),
                    status=1,
                    punch=0,
                    uid=1,
                ),
                attendance.Attendance(
                    user_id=1,
                    timestamp=datetime(2022, 1, 1, 11, 0, 1),
                    status=1,
                    punch=1,
                    uid=1,
                ),
